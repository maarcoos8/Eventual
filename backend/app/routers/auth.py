...
async def auth_google(request: Request):
    ...
    try:
        ...
        # Registrar la sesión en el log
        caducidad = datetime.utcnow() + timedelta(minutes=30)
        session_log = SessionLogCreate(
            usuario=user.email,
            caducidad=caducidad,
            token=access_token
        )
        await SessionLogCRUD.create(session_log)
        logging.info(f"Sesión registrada para: {email}")
